...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("working")
        await self.highrise.walk_to(Position(18.5 , 4.0 , 14.5 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.send_whisper(user.id,f"✨️Welcome {user.username}, spin the grab and go home.")

        await self.highrise.send_whisper(user.id,f"✨️Use: !emote list for list of emotes or 1-97, and !help For More Informations.")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username == "Khafiy":
              parts = message.split(" ")
              if len(parts) != 2:
                  await self.highrise.send_message(user.id, "❌️Invalid command use: !tipall [amount]")
                  return
        ...
        if message.lower().startswith("!tipme ") and user.username== "Khafiy":
                try:
